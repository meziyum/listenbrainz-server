...
def load_instant():
    ...
    return render_template(
        ...
        props=orjson.dumps({"playlist": serialize_jspf(playlist)})
    )
...
def load_release(release_mbid):
    ...
    return render_template(
        "player/player-page.html",
        props=orjson.dumps({"playlist": serialize_jspf(playlist) if playlist is not None else {}})
    )
