...
class RedisListenStore:
    ...
    def put_playing_now(self, user_id, listen, expire_time):
        ...
        """
        cache.set(self.PLAYING_NOW_KEY + str(user_id), orjson.dumps(listen).encode('utf-8'), expirein=expire_time)
    ...
    def update_recent_listens(self, unique):
        ...
        for listen in unique:
            recent[orjson.dumps(listen.to_json()).encode('utf-8')] = float(listen.ts_since_epoch)
