...
def insert_new_releases_of_top_artists(user_id, year, data):
    with db.engine.connect() as connection:
        connection.execute(sqlalchemy.text("""
            ...
            ON CONFLICT (user_id, year)
          DO UPDATE SET data = statistics.year_in_music.data || EXCLUDED.data
        """), {"user_id": user_id, "year": year, "data": orjson.dumps(data)})
...
def handle_multi_large_insert(key, year, data):
    ...
    try:
        with connection.cursor() as cursor:
            values = [(user["user_id"], orjson.dumps(user["data"])) for user in data]
            execute_values(cursor, query, values)
        connection.commit()
    except psycopg2.errors.OperationalError:
        connection.rollback()
...
def handle_insert_top_stats(entity, year, data):
    ...
    try:
        with connection.cursor() as cursor:
            values = [(user["user_id"], user["count"], orjson.dumps(user["data"])) for user in data]
            execute_values(cursor, query, values)
        connection.commit()
    except psycopg2.errors.OperationalError:
        connection.rollback()
...
def insert_playlists(year, data):
    ...
    try:
        with connection.cursor() as cursor:
            execute_values(cursor, query, [(user, slug, orjson.dumps(playlist)) for user, slug, playlist in data])
        connection.commit()
    except psycopg2.errors.OperationalError:
        connection.rollback()
