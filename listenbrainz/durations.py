...
def create_length_cache_tmp():
    mb_conn = psycopg2.connect(current_app.config["MBID_MAPPING_DATABASE_URI"])
    lb_conn = psycopg2.connect(current_app.config["SQLALCHEMY_TIMESCALE_URI"])
    with mb_conn.cursor('recording_length_cursor', cursor_factory=DictCursor) as mb_curs, \
